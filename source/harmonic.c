...
int harmonic_indices(
  ...
  if (ppt->has_cls == _TRUE_) {
    ...
    /* YG: add EB: */
    /* note: this puts EB after BB in cl output */
    if (ppt->has_cl_cmb_polarization == _TRUE_) {
      phr->has_eb = _TRUE_;
...
int harmonic_compute_cl(
  ...
  for (index_q=0; index_q < ptr->q_size; index_q++) {
    ...
    */
    ...
    /* YG: add eb */
    if (_scalars_ && (phr->has_eb == _TRUE_))
      cl_integrand[index_q*cl_integrand_num_columns+1+phr->index_ct_eb]=
        primordial_pk[index_ic1_ic2]
        * 0.5*(transfer_ic1[ptr->index_tt_e] * transfer_ic2[ptr->index_tt_b] +
               transfer_ic1[ptr->index_tt_b] * transfer_ic2[ptr->index_tt_e])
        * factor;
