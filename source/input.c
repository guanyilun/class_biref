...
int input_read_parameters_spectra(struct file_content * pfc,
  ...
  /** YG: add birefringence related parameters */
  /** 4. birefringence */
  class_call(parser_read_string(pfc,"biref_beta_file",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  /* Complete set of parameters */
  if ((flag1 == _TRUE_)) {
    ptr->biref_has_file = _TRUE_;
      class_read_string("biref_beta_file",ptr->biref_beta_file);
  }
  /** end of birefringence related parameters */
