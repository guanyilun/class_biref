...
int transfer_init(
  ...
  */
  ...
  /* YG: variables for birefringence */
  FILE *biref_input_file;
  int biref_num_rows;
  int biref_file_status;
  double tmp1, tmp2, tmp3;  // dummy memory for loading the file
  double tau;
  int index_tau;
  int last_index;
  ...
  */
  ...
  /* ---------------------------------- */
  /* YG: begin axion biref modification */
  /* ---------------------------------- */
  class_alloc(ptr->biref_beta, sizeof(double) * tau_size_max, ptr->error_message); // to store the beta value at known tau sampling

  /* load beta from file and build interpolator */
  if (ptr->biref_has_file == _TRUE_) {
    biref_input_file = fopen(ptr->biref_beta_file,"r");  // FIXME
    class_test(biref_input_file == NULL,
                ptr->error_message,
                "Could not open file %s!", "beta.txt");  // FIXME

    /* Find size of table */
    for (biref_num_rows=0,biref_file_status=3; biref_file_status == 3; biref_num_rows++) {
      biref_file_status = fscanf(biref_input_file, "%lf %lf %lf", &tmp1, &tmp2, &tmp3);
    }
    rewind(biref_input_file);
    ptr->biref_beta_size = biref_num_rows-1;

    /* Allocate room for interpolation table */
    class_alloc(ptr->biref_tau_table, sizeof(double) * ptr->biref_beta_size, ptr->error_message);
    class_alloc(ptr->biref_beta_table, sizeof(double) * ptr->biref_beta_size, ptr->error_message);
    class_alloc(ptr->biref_beta_table_spline, sizeof(double) * ptr->biref_beta_size, ptr->error_message); // to store 2nd derivative for spline

    for (biref_num_rows=0; biref_num_rows < ptr->biref_beta_size; biref_num_rows++) {
      biref_file_status = fscanf(biref_input_file, "%lf %lf %lf",
                                 &tmp1, &ptr->biref_tau_table[biref_num_rows], &ptr->biref_beta_table[biref_num_rows]);
    }
    fclose(biref_input_file);

    /* use the beta table to interpolate the beta value at the current tau */
    class_call(array_spline_table_lines(ptr->biref_tau_table,
                                        ptr->biref_beta_size,
                                        ptr->biref_beta_table,
                                        1,
                                        ptr->biref_beta_table_spline,
                                        _SPLINE_EST_DERIV_,
                                        ptr->error_message),
               ptr->error_message,
               ptr->error_message);

    /* actually apply interpolation */
    for (index_tau=0; index_tau < tau_size_max; index_tau++) {
      tau = ppt->tau_sampling[index_tau];
      /* since beta is computed independently of the cosmology here,
        it is not unlikely that tau range computed in the table
        is not matched with the tau sampling in class, e.g.,
        tau_sampling may go out of bound w.r.t tau_table. However,
        it doesn't matter for us to be precise here because the
        interesting range of tau that's relevant to birefringence
        is rather early, i.e., recomb and reion. Hence, we are 
        allowed to be a bit sloppy at the boundary and will just
        extrapolate by duplicating latest beta. */
      if (tau > ptr->biref_tau_table[ptr->biref_beta_size-1]) {
        // deal with out of bound: repeating latest value
        class_test(index_tau == 0, ptr->error_message, "this is not possible, something is wrong");
        ptr->biref_beta[index_tau] = ptr->biref_beta[index_tau-1];
      } else {
        class_call(array_interpolate_spline(ptr->biref_tau_table,
                                            ptr->biref_beta_size,
                                            ptr->biref_beta_table,
                                            ptr->biref_beta_table_spline,
                                            1,
    ...
    // if no biref file is given, set beta to 0
    for (index_tau=0; index_tau < tau_size_max; index_tau++) {
      ptr->biref_beta[index_tau] = 0;
    }
  }
  /* ----------------------------------- */
  /* YG: end of axion biref modification */
  /* ----------------------------------- */
...
int transfer_free(
  ...
  if (ptr->has_cls == _TRUE_) {
    ...
    if (ptr->biref_beta_size > 0) {
      free(ptr->biref_tau_table);
      free(ptr->biref_beta_table);
      free(ptr->biref_beta_table_spline);
      free(ptr->biref_beta);
    }
  }
...
int transfer_indices(
  ...
  // YG: probably this will break the vector and tensor indices, but for my
  // purpose, I only need the scalar ones, so this is fine.
    ...
    for (index_tt=0;index_tt<ptr->tt_size[index_md];index_tt++) {
      ...
      if (_scalars_) {
        ...
        if ((ppt->has_cl_cmb_polarization == _TRUE_) && 
            ((index_tt == ptr->index_tt_e) || (index_tt == ptr->index_tt_b)))
          l_max=ppt->l_scalar_max;
...
int transfer_can_be_neglected(
  ...
  *neglect = _FALSE_;
  ...
  if (_scalars_) {
    ...
    // YG: add B-mode: use the same precision setting as E-mode (hacky)
    else if ((ppt->has_cl_cmb_polarization == _TRUE_) && (index_tt == ptr->index_tt_b) && (l < (k-ppr->transfer_neglect_delta_k_S_e)*ra_rec)) *neglect = _TRUE_;
  ...
  */
  ...
  case SCALAR_POLARISATION_E:
    ...
    for (j=0; j<x_size; j++)
      ...
      // YG: add cos(2 beta)
      radial_function[x_size-1-j] = factor*cscKgen[x_size-1-j]*cscKgen[x_size-1-j]*Phi[j]*rescale_function[j] * cos(2*ptr->biref_beta[x_size-1-j]);
    break;
  // YG: add scalar B-mode
  case SCALAR_POLARISATION_B:
    class_call(interpolate_Phi(pHIS, x_size, index_l, chireverse, Phi, ptr->error_message),
               ptr->error_message, ptr->error_message);
    //hyperspherical_Hermite_interpolation_vector(pHIS, x_size, index_l, chireverse, Phi, NULL, NULL);
    s2 = sqrt(1.0-3.0*K/k2);
    ...
    for (j=0; j<x_size; j++)
      // radial_function[x_size-1-j] = factor*cscKgen[x_size-1-j]*cscKgen[x_size-1-j]*Phi[j]*rescale_function[j];
      // YG: add sin(2 beta)
      radial_function[x_size-1-j] = factor*cscKgen[x_size-1-j]*cscKgen[x_size-1-j]*Phi[j]*rescale_function[j] * sin(2*ptr->biref_beta[x_size-1-j]);
    break;
  ...
  if (_scalars_) {
    ...
    if (ppt->has_cl_cmb_polarization == _TRUE_) {
      ...
      if (index_tt == ptr->index_tt_b) {
        *radial_type = SCALAR_POLARISATION_B;
      }
